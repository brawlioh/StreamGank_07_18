...
class VideoQueueManager {
    constructor() {
        ...
        // 🔇 LOG THROTTLING: Prevent spam of concurrent processing messages
        this.lastSkipLogTime = new Map(); // Track last log time per job
        this.skipLogThrottle = 30000; // Only log once every 30 seconds per job
        ...
        this.cleanupInterval = setInterval(
            () => {
                ...
                // 🔇 CLEANUP: Clear old throttling entries to prevent memory leaks
                const now = Date.now();
                const maxAge = this.skipLogThrottle * 2; // Keep entries for 60 seconds max

                for (const [jobId, lastTime] of this.lastSkipLogTime.entries()) {
                    if (now - lastTime > maxAge) {
                        this.lastSkipLogTime.delete(jobId);
                    }
                }
            },
    ...
    async sendWebhookNotification(event, job) {
        ...
        try {
            ...
            const webhookData = {
                ...
                processing_duration_ms: job.completedAt && job.startedAt ? new Date(job.completedAt) - new Date(job.startedAt) : null,
            };
    ...
    async executeWithPool(operation, args = [], operationName = "redis_operation") {
        ...
        for (let attempt = 0; attempt < this.poolSize; attempt++) {
            try {
                ...
                console.warn(`⚠️ Redis ${operationName} failed on attempt ${attempt + 1}/${this.poolSize}:`, error.message);
        ...
        if (this.errorCount >= this.maxErrorsBeforeAlert) {
            console.error(`🚨 PRODUCTION ALERT: Redis error count exceeded ${this.maxErrorsBeforeAlert} in last ${this.errorResetInterval / 1000}s`);
        }
            ...
            try {
                ...
                if (jobData && jobData[1]) {
                    ...
                    if (!this.concurrentProcessing && this.activeJobs.size > 0) {
                        // 🔇 THROTTLE LOG: Only log once every 30 seconds per job to reduce spam
                        const now = Date.now();
                        const lastLogTime = this.lastSkipLogTime.get(job.id) || 0;

                        if (now - lastLogTime > this.skipLogThrottle) {
                            console.log(`⚠️ Job ${job.id} queued - waiting for current job to complete (${this.activeJobs.size} active)`);
                            this.lastSkipLogTime.set(job.id, now);
                        }
    ...
    async processJobAsync(job) {
        try {
            ...
            if (finalJob && finalJob.status === "completed") {
                const duration = finalJob.completedAt && finalJob.startedAt ? (new Date(finalJob.completedAt) - new Date(finalJob.startedAt)) / 1000 : null;
    ...
    async processJob(job) {
        ...
        try {
            ...
            if (result.pausedAfterExtraction) {
            ...
            } else if (result.status === "heygen_timeout") {
                // 🚨 NEW: Handle HeyGen timeout gracefully
                console.log(`⏰ HeyGen timeout detected for job ${job.id} - job saved for retry`);

                job.status = "failed";
                job.progress = 70; // Made it through most steps
                job.currentStep = "⏰ HeyGen video processing timed out - can be retried";
                job.error = "HeyGen processing timeout - servers may be busy";
                job.retryable = true; // Mark as retryable
                job.heygenTimeout = true; // Special flag for timeout
                job.videoIds = result.video_ids; // Save for potential recovery

                // Remove from processing queue
                await this.lremAsync(this.keys.processing, 1, JSON.stringify(job));
                await this.rpushAsync(this.keys.failed, JSON.stringify(job));
                await this.updateJob(job);

                // Send webhook notification for timeout (not permanent failure)
                this.sendWebhookNotification("job.heygen_timeout", job);

                // Add helpful logs
                this.addJobLog(job.id, "⏰ HeyGen video processing timed out after extended wait", "warning");
                this.addJobLog(job.id, "💡 This is likely due to high server load - job can be retried", "info");
                this.addJobLog(job.id, `🔍 Video IDs saved for recovery: ${Object.keys(result.video_ids || {}).join(", ")}`, "info");
                ...
                if (result.workflowFullyComplete) {
                    ...
                    this.addJobLog(job.id, `⚠️ Workflow may be incomplete - Step 7: ${result.step7Completed}, Workflow message: ${result.workflowCompleted}`, "warning");
                    this.addJobLog(job.id, `🔍 Manual verification needed for Creatomate ID: ${result.creatomateId}`, "info");
                }
            } else {
    ...
    shouldRetryError(errorMessage, currentRetryCount, maxRetries) {
        ...
        if (error.includes("critical workflow") || error.includes("movie processing failed") || error.includes("database extraction failed") || error.includes("no movies found") || error.includes("workflow failed") || error.includes("script execution failed")) {
            return {
                retry: false,
        ...
        // 🟡 SMART RETRY - HeyGen Timeouts (special handling)
        if (error.includes("heygen") && (error.includes("timeout") || error.includes("processing timeout"))) {
            return {
                retry: currentRetryCount < Math.min(maxRetries, 3), // Allow 3 retries for HeyGen timeouts
                reason: "HeyGen timeout - extended retry with longer backoff",
                delaySeconds: Math.pow(2, currentRetryCount + 3) * 60, // 8, 16, 32 minutes - longer waits
                solution: "HeyGen servers may be overloaded - longer wait times help",
            };
        }
                        ...
                        for (const pattern of stepPatterns) {
                            ...
                            if (matchesAll) {
                                ...
                                if (pattern.patterns.includes("🎉 WORKFLOW COMPLETED SUCCESSFULLY")) {
                                    ...
                                    console.log(`🎉 Workflow completion detected for job ${job.id} - All steps: ${allStepsCompleted}`);
                                }
    ...
    /**
     * 🔧 HELPER: Remove a job from any queue by job ID
     * @param {string} queueKey - Redis key for the queue
     * @param {string} jobId - Job ID to remove
     */
    async removeFromQueue(queueKey, jobId) {
        try {
            // Get all items in the specified queue
            const queueItems = await this.lrangeAsync(queueKey, 0, -1);
            let removed = false;
            ...
            for (let i = 0; i < queueItems.length; i++) {
                ...
                if (item.id === jobId) {
                    // Remove this specific item from the list
                    await this.lremAsync(queueKey, 1, queueItems[i]);
            ...
            if (!removed) {
                console.log(`ℹ️ Job ${jobId} not found in queue: ${queueKey}`);
            }
        } catch (error) {
            console.error(`❌ Failed to remove job ${jobId} from queue ${queueKey}:`, error);
        }
    }
    ...
    async cancelJob(jobId) {
        try {
            ...
            // 🔧 CRITICAL FIX: Remove from ALL queues (pending, processing, completed)
            console.log(`🧹 Cancelling job ${jobId} - removing from all queues...`);
            console.log(`   Current job status: ${job.status}`);

            // Remove from pending queue (this was the missing piece!)
            await this.removeFromQueue(this.keys.pending, jobId);

            // Remove from processing queue if it's there
            await this.removeFromProcessingQueue(job);

            // Remove from completed queue if it's there
            await this.removeFromQueue(this.keys.completed, jobId);

            console.log(`✅ Job ${jobId} removed from all queues`);
    ...
    async deleteJob(jobId) {
        try {
            ...
            if (!["completed", "failed", "cancelled"].includes(job.status)) {
                throw new Error(`Cannot delete job with status: ${job.status}. Only completed, failed, or cancelled jobs can be deleted.`);
            }
        ...
        try {
            ...
            if (this.jobCache && this.jobCache.size > 0) {
                ...
                if (cacheCleanedCount > 0) {
                    console.log(`🧹 Cleaned up ${cacheCleanedCount} expired job cache entries (${this.jobCache.size} remaining)`);
                }
            }
    ...
    addJobLog(jobId, message, type = "info") {
        ...
        if (type === "error" || message.includes("STEP") || message.includes("completed") || message.includes("Video is ready")) {
    ...
    async startCreatomateMonitoring(jobId, creatomateId) {
        ...
        if (this.activeMonitoring.size >= this.maxConcurrentMonitoring) {
            ...
            console.log(`📋 Queued monitoring: ${jobId} (${this.monitoringQueue.length} in queue, ${this.activeMonitoring.size} active)`);
            return;
        }
        ...
        // Single startup log
        console.log(`🎬 Starting PROJECT-LEVEL webhook backup monitoring [${this.activeMonitoring.size}/${this.maxConcurrentMonitoring}]: ${jobId} → ${creatomateId}`);
        console.log(`🔗 Primary: Project-level webhook (ALL statuses) | 🔄 Backup: ${maxAttempts} checks over ${Math.round((maxAttempts * checkInterval) / 60000)}min`);
        ...
        const checkStatus = async () => {
            ...
            try {
                ...
                if (response.success && (response.status === "completed" || response.status === "succeeded") && response.url) {
                    ...
                    if (shouldLog) {
                        console.log(`⏳ Backup check ${jobId}: ${status} [${attempts}/${maxAttempts}] (webhook primary)`);
    ...
    async markJobAsRenderTimeout(jobId, creatomateId) {
        try {
            ...
            this.addJobLog(jobId, `🔍 Check render manually: python main.py --check-creatomate ${creatomateId}`, "info");
