import{U as r,A as d,D as u,R as m}from"./RealtimeService.BdD4_qZh.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function t(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(a){if(a.ep)return;a.ep=!0;const o=t(a);fetch(a.href,o)}})();class T extends EventTarget{constructor(){super(),this.routes=new Map,this.currentRoute=null,this.currentParams={},this.isInitialized=!1,this.basePath=""}init(){this.isInitialized||(window.addEventListener("popstate",e=>{this.handleLocationChange()}),this.handleLocationChange(),this.isInitialized=!0)}addRoute(e,t,s={}){const a=this.pathToRegex(e);this.routes.set(e,{pattern:a,handler:t,params:this.extractParams(e),title:s.title||"StreamGank",requiresAuth:s.requiresAuth||!1,metadata:s.metadata||{}}),console.log(`🛤️ Route registered: ${e}`)}navigate(e,t={}){const{replace:s=!1,state:a=null}=t;s?window.history.replaceState(a,"",e):window.history.pushState(a,"",e),this.handleLocationChange()}back(){window.history.back()}forward(){window.history.forward()}handleLocationChange(){const e=window.location.pathname,t=this.matchRoute(e);if(t){const{route:s,params:a,routeKey:o}=t,n=this.currentRoute;this.currentRoute=o,this.currentParams=a,s.title&&(document.title=s.title),this.dispatchEvent(new CustomEvent("routeChange",{detail:{path:e,route:o,params:a,previousRoute:n,metadata:s.metadata}}));try{s.handler(a,e),console.log(`🛤️ Navigated to: ${e} (${o})`)}catch(i){console.error("🛤️ Route handler error:",i),this.dispatchEvent(new CustomEvent("routeError",{detail:{path:e,error:i}}))}}else this.handle404(e)}matchRoute(e){for(const[t,s]of this.routes.entries()){const a=e.match(s.pattern);if(a){const o={};return s.params.forEach((n,i)=>{o[n]=a[i+1]}),{route:s,params:o,routeKey:t,match:a}}}return null}pathToRegex(e){const t=e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&").replace(/\\:([^/]+)/g,"([^/]+)");return new RegExp(`^${t}$`)}extractParams(e){const t=[],s=e.matchAll(/:([^/]+)/g);for(const a of s)t.push(a[1]);return t}handle404(e){console.warn(`🛤️ No route found for: ${e}`),this.dispatchEvent(new CustomEvent("notFound",{detail:{path:e}})),e!=="/"&&e!=="/dashboard"&&this.navigate("/dashboard",{replace:!0})}generateUrl(e,t={}){let s=e;for(const[a,o]of Object.entries(t))s=s.replace(`:${a}`,encodeURIComponent(o));return s}getCurrentRoute(){return{path:window.location.pathname,route:this.currentRoute,params:this.currentParams,hash:window.location.hash,search:window.location.search}}isCurrentRoute(e){return this.currentRoute===e}updateQuery(e){const t=new URL(window.location);for(const[s,a]of Object.entries(e))a==null?t.searchParams.delete(s):t.searchParams.set(s,a);this.navigate(t.pathname+t.search,{replace:!0})}getQuery(){const e={},t=new URLSearchParams(window.location.search);for(const[s,a]of t.entries())e[s]=a;return e}cleanup(){window.removeEventListener("popstate",this.handleLocationChange),this.routes.clear(),this.isInitialized=!1,console.log("🛤️ Router cleaned up")}emit(e,t){const s=new CustomEvent(e,{detail:t});this.dispatchEvent(s)}}const h=new T;class I extends EventTarget{constructor(){super(),this.activeJobs=new Map,this.jobHistory=new Map,this.currentJob=null,this.maxJobHistory=100,this.monitoringInterval=5e3,this.monitoringTimer=null,this.isGenerationActive=!1,this.creatomateMessages=new Set}init(){this.setupEventListeners()}setupEventListeners(){window.addEventListener("beforeunload",()=>{this.cleanup()})}async startVideoGeneration(e){try{if(this.validateGenerationParams(e),this.isGenerationActive)throw new Error("Another video generation is already in progress");this.isGenerationActive=!0,r.showProgress(),r.disableGenerateButton("Starting generation..."),r.addStatusMessage("info","🚀","Starting video generation..."),this.creatomateMessages.clear();const t=await d.generateVideo(e);if(!t.success)throw new Error(t.message||"Failed to start video generation");const s=this.createJobObject(t,e);return this.activeJobs.set(s.id,s),this.currentJob=s,this.startJobMonitoring(s.id),r.addStatusMessage("success","✅",`Job queued successfully! ${s.queuePosition?`Position: ${s.queuePosition}`:""}`),r.updateProgress(5,"Job queued, waiting to start..."),this.dispatchEvent(new CustomEvent("jobStarted",{detail:{job:s}})),console.log(`💼 Job started: ${s.id}`),{success:!0,job:s}}catch(t){throw console.error("❌ Failed to start video generation:",t),this.resetGenerationState(),r.addStatusMessage("error","❌",`Failed to start generation: ${t.message}`),this.dispatchEvent(new CustomEvent("jobError",{detail:{error:t}})),t}}createJobObject(e,t){return{id:e.jobId,params:t,status:"pending",progress:0,createdAt:new Date().toISOString(),startedAt:null,completedAt:null,queuePosition:e.queuePosition||0,error:null,result:null,creatomateId:null,videoUrl:null}}validateGenerationParams(e){const s=["country","platform","genre","contentType"].filter(a=>!e[a]);if(s.length>0)throw new Error(`Missing required parameters: ${s.join(", ")}`);console.log("✅ Parameters validated:",e)}async startJobMonitoring(e){this.monitoringTimer&&clearInterval(this.monitoringTimer),console.log(`👀 Started monitoring job: ${e}`),this.monitoringTimer=setInterval(async()=>{try{await this.updateJobStatus(e)}catch(t){console.error("❌ Job monitoring error:",t),this.consecutiveErrors>3&&(this.stopJobMonitoring(),r.addStatusMessage("warning","⚠️","Job monitoring stopped due to repeated errors"))}},this.monitoringInterval)}stopJobMonitoring(){this.monitoringTimer&&(clearInterval(this.monitoringTimer),this.monitoringTimer=null,console.log("⏹️ Job monitoring stopped"))}async updateJobStatus(e){if(this.activeJobs.get(e))try{const s=await d.getJobStatus(e);s.success&&s.job&&this.processJobUpdate(s.job)}catch(s){throw console.error(`❌ Failed to update job status for ${e}:`,s),s}}processJobUpdate(e){const t=this.activeJobs.get(e.id);if(!t)return;const s=t.status,a=t.progress;Object.assign(t,{status:e.status,progress:e.progress||0,currentStep:e.currentStep,startedAt:e.startedAt||t.startedAt,completedAt:e.completedAt,error:e.error,result:e,creatomateId:e.creatomateId,videoUrl:e.videoUrl}),s!==t.status&&this.handleJobStatusChange(t,s),a!==t.progress&&this.updateJobProgress(t),t.creatomateId&&!t.videoUrl&&t.status==="completed"&&this.startCreatomateMonitoring(t),this.dispatchEvent(new CustomEvent("jobUpdated",{detail:{job:t,previousStatus:s}})),console.log(`💼 Job ${t.id} updated: ${t.status} (${t.progress}%)`)}handleJobStatusChange(e,t){switch(e.status){case"processing":t==="pending"&&(r.addStatusMessage("info","⚡","Job started processing!"),e.startedAt=new Date().toISOString());break;case"completed":this.handleJobCompletion(e);break;case"failed":this.handleJobFailure(e);break;case"cancelled":this.handleJobCancellation(e);break}}handleJobCompletion(e){console.log(`✅ Job completed: ${e.id}`),e.videoUrl?this.finishSuccessfulGeneration(e):e.creatomateId?(r.updateProgress(90,"Python script completed, video rendering..."),r.addStatusMessage("info","🎬",`Video rendering started (ID: ${e.creatomateId}). Monitoring progress...`),this.startCreatomateMonitoring(e)):(r.addStatusMessage("warning","⚠️","Job completed but video URL not yet available"),this.moveJobToHistory(e)),this.dispatchEvent(new CustomEvent("jobCompleted",{detail:{job:e}}))}startCreatomateMonitoring(e){let t=0;const s=40,a=async()=>{t++;try{const o=await d.getCreatomateStatus(e.creatomateId);if(o.success&&o.videoUrl)e.videoUrl=o.videoUrl,e.result.videoUrl=o.videoUrl,this.finishSuccessfulGeneration(e);else if(o.success&&o.status){const n=o.status.toLowerCase(),i=n.charAt(0).toUpperCase()+n.slice(1);if(t%4===0){const p=`rendering-update-${Math.floor(t/4)}`;this.creatomateMessages.has(p)||(r.addStatusMessage("info","⏳",`Video status: ${i}... (${t}/${s})`),this.creatomateMessages.add(p))}let c=90+t/s*10;(n.includes("render")||n.includes("process"))&&(c=Math.min(95,c)),r.updateProgress(c,`Rendering: ${i}`),t<s?setTimeout(()=>a(),3e4):this.handleCreatomateTimeout(e)}else this.handleCreatomateError(e,o.message,t,s,a)}catch(o){this.handleCreatomateNetworkError(e,o,t,s,a)}};a()}handleCreatomateTimeout(e){const t="creatomate-timeout";this.creatomateMessages.has(t)||(r.addStatusMessage("warning","⚠️",'Video rendering is taking longer than expected. Use "Check Status" to monitor manually.'),this.creatomateMessages.add(t)),r.enableGenerateButton(),this.isGenerationActive=!1}handleCreatomateError(e,t,s,a,o){const n=`creatomate-error-${t}`;this.creatomateMessages.has(n)||(r.addStatusMessage("error","❌",`Render status check failed: ${t||"Unknown error"}`),this.creatomateMessages.add(n)),s<a?setTimeout(()=>o(),3e4):(r.addStatusMessage("error","❌","Unable to check render status after multiple attempts."),this.moveJobToHistory(e))}handleCreatomateNetworkError(e,t,s,a,o){console.error("Creatomate status check error:",t);const n=`network-error-${s}`;s%3===0&&!this.creatomateMessages.has(n)&&(r.addStatusMessage("warning","⚠️",`Network error checking render status (attempt ${s})`),this.creatomateMessages.add(n)),s<a?setTimeout(()=>o(),3e4):(r.addStatusMessage("error","❌","Network errors prevented render status monitoring."),this.moveJobToHistory(e))}finishSuccessfulGeneration(e){r.updateProgress(100,"Generation completed!"),r.addStatusMessage("success","🎉","Video generation completed successfully!"),r.displayVideo({jobId:e.id,videoUrl:e.videoUrl,creatomateId:e.creatomateId,timestamp:new Date().toLocaleString()}),this.moveJobToHistory(e),this.resetGenerationState()}handleJobFailure(e){console.error(`❌ Job failed: ${e.id}`,e.error),r.updateProgress(0,"Generation failed"),r.addStatusMessage("error","❌",`Generation failed: ${e.error||"Unknown error"}`,!1),this.moveJobToHistory(e),this.dispatchEvent(new CustomEvent("jobFailed",{detail:{job:e}})),this.resetGenerationState()}handleJobCancellation(e){console.log(`⏹️ Job cancelled: ${e.id}`),r.addStatusMessage("warning","⏹️","Job was cancelled"),this.moveJobToHistory(e),this.dispatchEvent(new CustomEvent("jobCancelled",{detail:{job:e}})),this.resetGenerationState()}updateJobProgress(e){e===this.currentJob&&r.updateProgress(e.progress,e.currentStep||"Processing...")}resetGenerationState(){r.hideProgress(),r.enableGenerateButton(),this.isGenerationActive=!1,this.stopJobMonitoring()}async cancelJob(e){try{const t=await d.cancelJob(e);if(t.success)return r.addStatusMessage("info","⏹️","Job cancellation requested"),!0;throw new Error(t.message||"Failed to cancel job")}catch(t){return console.error("❌ Failed to cancel job:",t),r.addStatusMessage("error","❌",`Failed to cancel job: ${t.message}`),!1}}stopVideoGeneration(){this.currentJob&&this.cancelJob(this.currentJob.id),this.resetGenerationState(),r.addStatusMessage("warning","⏹️","Video generation stopped")}moveJobToHistory(e){this.activeJobs.delete(e.id),this.jobHistory.set(e.id,{...e,movedToHistoryAt:new Date().toISOString()}),this.currentJob&&this.currentJob.id===e.id&&(this.currentJob=null),this.limitJobHistory()}limitJobHistory(){if(this.jobHistory.size>this.maxJobHistory){const e=Array.from(this.jobHistory.entries()),t=e.slice(0,e.length-this.maxJobHistory);t.forEach(([s])=>{this.jobHistory.delete(s)}),console.log(`🧹 Cleaned up ${t.length} old job records`)}}getJob(e){return this.activeJobs.get(e)||this.jobHistory.get(e)||null}getActiveJobs(){return Array.from(this.activeJobs.values())}getJobStats(){var e;return{active:this.activeJobs.size,history:this.jobHistory.size,total:this.activeJobs.size+this.jobHistory.size,currentJob:((e=this.currentJob)==null?void 0:e.id)||null,isMonitoring:!!this.monitoringTimer,isGenerationActive:this.isGenerationActive}}cleanup(){this.stopJobMonitoring(),console.log("🧹 Job Manager cleaned up")}}const g=new I;class M{constructor(){this.genresByCountry={},this.templatesByGenre={},this.platformsByCountry={},this.formState={country:"",platform:"",genre:"",template:"",contentType:""},this.isValidating=!1,this.validationCache=new Map,this.isInitialized=!1}async init(){if(this.isInitialized){console.log("📋 FormManager already initialized, skipping...");return}await this.loadFormConfiguration(),this.setupEventListeners(),this.initializeFormState(),this.isInitialized=!0,console.log("✅ FormManager initialized successfully")}async loadFormConfiguration(){this.loadGenresByCountry(),this.loadTemplatesByGenre(),await this.initializePlatformData()}loadGenresByCountry(){this.genresByCountry={FR:{"Action & Aventure":"Action & Adventure",Animation:"Animation",Comédie:"Comedy","Comédie Romantique":"Romantic Comedy","Crime & Thriller":"Crime & Thriller",Documentaire:"Documentary",Drame:"Drama",Fantastique:"Fantasy","Film de guerre":"War Movies",Histoire:"History",Horreur:"Horror","Musique & Comédie Musicale":"Music & Musical Comedy","Mystère & Thriller":"Mystery & Thriller","Pour enfants":"Kids","Reality TV":"Reality TV","Réalisé en Europe":"Made in Europe","Science-Fiction":"Science Fiction","Sport & Fitness":"Sport & Fitness",Western:"Western"},US:{"Action & Adventure":"Action & Adventure",Animation:"Animation",Comedy:"Comedy",Crime:"Crime",Documentary:"Documentary",Drama:"Drama",Fantasy:"Fantasy",History:"History",Horror:"Horror","Kids & Family":"Kids & Family","Made in Europe":"Made in Europe","Music & Musical":"Music & Musical","Mystery & Thriller":"Mystery & Thriller","Reality TV":"Reality TV","Romance Movies":"Romance Movies","Science Fiction":"Science Fiction","Sport & Fitness":"Sport & Fitness","Stand-up Comedy":"Stand-up Comedy",Western:"Western"}}}loadTemplatesByGenre(){this.templatesByGenre={Horror:"ed21a309a5c84b0d873fde68642adea3",Horreur:"ed21a309a5c84b0d873fde68642adea3",Comedy:"15d9eadcb46a45dbbca1834aa0a23ede",Comédie:"15d9eadcb46a45dbbca1834aa0a23ede","Stand-up Comedy":"15d9eadcb46a45dbbca1834aa0a23ede","Action & Adventure":"e44b139a1b94446a997a7f2ac5ac4178","Action & Aventure":"e44b139a1b94446a997a7f2ac5ac4178",default:"cc6718c5363e42b282a123f99b94b335"}}async initializePlatformData(){const e=document.getElementById("country");if(e){const s=e.value||"US";await this.updatePlatformDropdown(s)}const t=e?e.value:"US";await this.updateGenreDropdown(t),this.updateTemplateDropdown(),this.setDefaultSelections(),this.refreshFormState(),console.log("✅ Platform data initialization complete")}async updatePlatformDropdown(e){const t=`platforms_${e}`;if(!this.validationCache.has(t))try{const s=await d.get(`/api/platforms/${e}`);s.success&&s.platforms?(this.populatePlatformSelect(s.platforms),this.validationCache.set(t,!0),console.log("✅ Platform dropdown update completed")):console.error("❌ Invalid platform API response:",s)}catch(s){console.error("❌ Failed to load platforms:",s)}}async updateGenreDropdown(e){const t=`genres_${e}`;if(this.validationCache.has(t)){console.log(`📋 Using cached genres for ${e}`);return}try{console.log(`📋 Loading genres for ${e}...`);const s=await d.get(`/api/genres/${e}`);s.success&&s.genres?(console.log(`📋 API returned ${s.genres.length} genres:`,s.genres),this.populateGenreSelect(s.genres),this.validationCache.set(t,!0),console.log("✅ Genre dropdown update completed")):console.error("❌ Invalid genre API response:",s)}catch(s){console.error("❌ Failed to load genres:",s)}}updateTemplateDropdown(){const e=document.getElementById("template");if(!e)return;e.innerHTML='<option value="">Select Template...</option>',[{value:"cc6718c5363e42b282a123f99b94b335",text:"Default Template"},{value:"ed21a309a5c84b0d873fde68642adea3",text:"Horror/Thriller Cinematic"},{value:"15d9eadcb46a45dbbca1834aa0a23ede",text:"Comedy Upbeat"},{value:"e44b139a1b94446a997a7f2ac5ac4178",text:"Action Adventure"}].forEach(s=>{const a=document.createElement("option");a.value=s.value,a.textContent=s.text,e.appendChild(a)}),e.value="cc6718c5363e42b282a123f99b94b335"}setupEventListeners(){if(this.eventListenersSetup)return;const e=document.getElementById("country");e&&e.addEventListener("change",i=>{this.handleCountryChange(i.target.value)});const t=document.getElementById("platform");t&&t.addEventListener("change",i=>{this.handlePlatformChange(i.target.value)});const s=document.getElementById("genre");s&&s.addEventListener("change",i=>{this.handleGenreChange(i.target.value)});const a=document.getElementById("template");a&&a.addEventListener("change",i=>{this.handleTemplateChange(i.target.value)});const o=document.querySelectorAll('input[name="contentType"]');o&&Array.from(o).forEach(i=>{i.addEventListener("change",c=>{c.target.checked&&this.handleContentTypeChange(c.target.value)})});const n=document.getElementById("generate-video");n&&n.addEventListener("click",i=>{i.preventDefault(),this.handleFormSubmit()}),this.eventListenersSetup=!0}initializeFormState(){const e=document.getElementById("country");e&&!e.value&&(e.value="US"),this.ensureContentTypeSelected(),console.log("📋 Form state initialization complete")}ensureContentTypeSelected(){const e=document.querySelectorAll('input[name="contentType"]'),t=Array.from(e).find(s=>s.checked);if(console.log("📋 Content Type Radios found:",e.length),console.log("📋 Already checked:",t?t.value:"none"),!t&&e.length>0){const s=Array.from(e).find(a=>a.value==="Serie");s?(s.checked=!0,console.log("📋 Force-selected Serie (TV Shows) radio button")):(e[0].checked=!0,console.log("📋 Force-selected first radio button:",e[0].value))}}refreshFormState(){const e=u.getFormData();Object.assign(this.formState,e),console.log("📋 Form state updated"),console.log("📋 Final form data:",this.formState),this.updatePreview()}setDefaultSelections(){let e=!1;const t=document.getElementById("platform");if(t&&t.children.length>1&&t.selectedIndex===0){let a=-1;for(let o=1;o<t.options.length;o++)if(t.options[o].value.toLowerCase().includes("netflix")){a=o;break}t.selectedIndex=a>0?a:1,this.formState.platform=t.value,console.log("📋 Set default platform:",this.formState.platform),e=!0}const s=document.getElementById("genre");if(s&&s.children.length>1&&s.selectedIndex===0){let a=-1;for(let o=1;o<s.options.length;o++)if(s.options[o].value.toLowerCase().includes("horror")){a=o;break}s.selectedIndex=a>0?a:1,this.formState.genre=s.value,console.log("📋 Set default genre:",this.formState.genre),e=!0,this.updateTemplates(this.formState.genre)}e&&console.log("📋 Defaults set")}manualRefresh(){return console.log("📋 Manual form refresh triggered"),this.refreshFormState(),this.formState}async handleCountryChange(e){console.log(`📋 Country changed: ${e}`),this.formState.country=e,this.resetPlatformSelection(),this.resetGenreSelection(),this.resetTemplateSelection(),e&&await this.updatePlatforms(e),this.updatePreview()}async handlePlatformChange(e){console.log(`📋 Platform changed: ${e}`),this.formState.platform=e,this.resetGenreSelection(),this.resetTemplateSelection();const t=`genres_${this.formState.country}_${e}`;this.validationCache.delete(t),this.formState.country&&e&&await this.updateGenres(this.formState.country,e),this.updatePreview()}handleGenreChange(e){console.log(`📋 Genre changed: ${e}`),this.formState.genre=e,this.resetTemplateSelection(),e&&this.updateTemplates(e),this.updatePreview()}handleTemplateChange(e){console.log(`📋 Template changed: ${e}`),this.formState.template=e,this.updatePreview()}handleContentTypeChange(e){console.log(`📋 Content type changed: ${e}`),this.formState.contentType=e,this.updatePreview()}async updatePlatforms(e){try{const t=await d.getPlatforms(e);t.success&&t.platforms?this.populatePlatformSelect(t.platforms):this.populateDefaultPlatforms(e)}catch(t){console.error("❌ Failed to load platforms:",t),this.populateDefaultPlatforms(e)}}populatePlatformSelect(e){const t=document.getElementById("platform");if(!t){console.error("❌ Platform select element not found!");return}for(console.log("📋 Populating platforms:",e);t.children.length>1;)t.removeChild(t.lastChild);e.forEach(s=>{const a=document.createElement("option");a.value=s,a.textContent=s,t.appendChild(a)}),console.log("✅ Platform dropdown populated with",e.length,"options")}populateDefaultPlatforms(e){const t=[{value:"Netflix",name:"Netflix"},{value:"Prime Video",name:"Prime Video"},{value:"Disney+",name:"Disney+"},{value:"Apple TV+",name:"Apple TV+"},{value:"HBO Max",name:"HBO Max"}];this.populatePlatformSelect(t)}async updateGenres(e,t){const s=`genres_${e}_${t}`;if(this.validationCache.has(s)){console.log(`📋 Using cached genres for ${e}/${t}`);return}try{console.log(`📋 Loading genres for ${e}/${t}...`);const a=await d.getGenres(e,t);if(a.success&&a.genres){console.log(`📋 API returned ${a.genres.length} genres for ${t}:`,a.genres),this.populateGenreSelect(a.genres),this.validationCache.set(s,!0);return}}catch(a){console.error("❌ Failed to load genres from API:",a)}console.log(`📋 Using fallback static genres for ${e}`),this.populateGenreSelectFromStatic(e)}populateGenreSelect(e){const t=document.getElementById("genre");if(!t){console.error("❌ Genre select element not found!");return}for(console.log("📋 Populating genres:",e);t.children.length>1;)t.removeChild(t.lastChild);e.forEach(s=>{const a=document.createElement("option");a.value=s,a.textContent=s,t.appendChild(a)}),console.log("✅ Genre dropdown populated with",e.length,"options")}populateGenreSelectFromStatic(e){const t=this.genresByCountry[e];if(!t)return;const s=document.getElementById("genre");if(s){for(;s.children.length>1;)s.removeChild(s.lastChild);Object.entries(t).forEach(([a,o])=>{const n=document.createElement("option");n.value=o,n.textContent=a,s.appendChild(n)})}}updateTemplates(e){const t=document.getElementById("template");if(!t)return;const s=this.getTemplateForGenre(e);Array.from(t.options).forEach(a=>{a.value===s&&(a.selected=!0,this.formState.template=s)}),console.log(`📋 Template auto-selected for genre '${e}': ${s}`)}getTemplateForGenre(e){if(this.templatesByGenre[e])return this.templatesByGenre[e];const t=e.toLowerCase();for(const[s,a]of Object.entries(this.templatesByGenre))if(s.toLowerCase()===t)return a;return this.templatesByGenre.default}resetPlatformSelection(){const e=document.getElementById("platform");if(e)for(e.selectedIndex=0;e.children.length>1;)e.removeChild(e.lastChild);this.formState.platform=""}resetGenreSelection(){const e=document.getElementById("genre");if(e)for(e.selectedIndex=0;e.children.length>1;)e.removeChild(e.lastChild);this.formState.genre=""}resetTemplateSelection(){const e=document.getElementById("template");e&&(e.selectedIndex=0),this.formState.template=""}validateForm(){const e=[],t=[];return this.formState.country||e.push("Country is required"),this.formState.platform||e.push("Platform is required"),this.formState.genre||e.push("Genre is required"),this.formState.contentType||e.push("Content type is required"),this.formState.template||t.push("No template selected - default will be used"),{isValid:e.length===0,errors:e,warnings:t}}async validateStreamGankUrl(e){if(!e||e.includes("Select all parameters"))return{valid:!1,message:"Please complete the form to generate a valid URL"};const t=`url:${e}`;if(this.validationCache.has(t))return this.validationCache.get(t);try{r.addStatusMessage("info","🔍","Validating URL..."),this.isValidating=!0;const s=await d.validateUrl(e),a={valid:s.success,message:s.message,moviesCount:s.moviesCount,timestamp:new Date().toISOString()};return this.validationCache.set(t,a),a.valid?r.addStatusMessage("success","✅",`URL validated! Found ${a.moviesCount} items`):r.addStatusMessage("error","❌",`URL validation failed: ${a.message}`),a}catch(s){console.error("❌ URL validation error:",s);const a={valid:!1,message:s.message||"Validation failed",timestamp:new Date().toISOString()};return r.addStatusMessage("error","❌",`Validation error: ${a.message}`),a}finally{this.isValidating=!1}}async handleFormSubmit(){try{this.updateFormStateFromDOM();const e=this.validateForm();if(!e.isValid){e.errors.forEach(a=>{r.addStatusMessage("error","❌",a)});return}e.warnings.forEach(a=>{r.addStatusMessage("warning","⚠️",a)});const t=this.generateStreamGankUrl(),s=await this.validateStreamGankUrl(t);if(!s.valid)return;document.dispatchEvent(new CustomEvent("formSubmit",{detail:{formData:{...this.formState},previewUrl:t,validation:s}}))}catch(e){console.error("❌ Form submission error:",e),r.addStatusMessage("error","❌",`Form submission failed: ${e.message}`)}}updateFormStateFromDOM(){const e=u.getFormData();Object.assign(this.formState,e)}updatePreview(){r.updateFormPreviewFromState(this.formState),console.log("📋 Preview updated")}generateStreamGankUrl(){if(!this.formState.country||!this.formState.platform||!this.formState.contentType)return"Select all parameters to generate URL";const e="https://streamgank.com",t=new URLSearchParams;if(this.formState.country&&t.set("country",this.formState.country),this.formState.platform&&t.set("platforms",this.formState.platform.toLowerCase()),this.formState.genre&&this.formState.genre!=="all"){const a={Horreur:"Horror",Comédie:"Comedy","Action & Aventure":"Action",Animation:"Animation"}[this.formState.genre]||this.formState.genre;t.set("genres",a)}if(this.formState.contentType&&this.formState.contentType!=="all"){const a={movies:"Film",series:"Serie",tvshows:"Serie","tv-shows":"Serie"}[this.formState.contentType.toLowerCase()]||this.formState.contentType;t.set("type",a)}return`${e}?${t.toString()}`}getFormData(){return this.updateFormStateFromDOM(),{...this.formState}}setFormData(e){Object.assign(this.formState,e),Object.entries(e).forEach(([t,s])=>{const a=u.get(`${t}Select`)||u.get(t);a&&a.value!==void 0&&(a.value=s)}),this.updatePreview()}resetForm(){this.formState={country:"",platform:"",genre:"",template:"",contentType:""},["country","platform","genre","template"].forEach(t=>{const s=document.getElementById(t);s&&(s.selectedIndex=0)});const e=u.get("contentTypeRadios");e&&Array.from(e).forEach(t=>{t.checked=!1}),this.validationCache.clear(),this.updatePreview(),console.log("📋 Form reset")}getValidationState(){return{isValidating:this.isValidating,cacheSize:this.validationCache.size,lastValidation:null}}emit(e,t){console.log(`📤 FormManager emitting ${e}:`,t);const s=new CustomEvent(e,{detail:t});document.dispatchEvent(s)}}const y=new M;class A{constructor(){this.processData=new Map,this.isInitialized=!1,this.updateInterval=null}async init(){this.isInitialized||(console.log("🔧 ProcessTable initializing..."),await this.loadRecentJobs(),this.setupEventListeners(),this.startPeriodicUpdates(),this.isInitialized=!0,console.log("✅ ProcessTable initialized"))}setupEventListeners(){document.addEventListener("click",e=>{const t=e.target.closest(".btn-view-job");if(t){e.preventDefault();const s=t.dataset.jobId;console.log(`🎯 View button clicked for job: ${s}`),this.viewJob(s)}}),document.addEventListener("click",e=>{const t=e.target.closest(".btn-cancel-job");if(t){e.preventDefault();const s=t.dataset.jobId;console.log(`🚫 Cancel button clicked for job: ${s}`),this.cancelJob(s)}}),document.addEventListener("click",e=>{const t=e.target.closest(".btn-delete-job");if(t){e.preventDefault();const s=t.dataset.jobId;console.log(`🗑️ Delete button clicked for job: ${s}`),this.deleteJob(s)}})}async loadRecentJobs(){if(document.hidden){console.log("📋 ProcessTable: Skipping job load - page not visible (anti-spam)");return}try{console.log("📋 ProcessTable: Loading jobs from /api/queue/jobs...");const e=await d.get("/api/queue/jobs");console.log("📋 ProcessTable: API response:",e),e.success&&e.jobs?(console.log(`📋 ProcessTable: Received ${e.jobs.length} jobs`),console.log("📋 ProcessTable: Raw job data:",e.jobs[0]),this.processData.clear(),e.jobs.forEach(t=>{var a,o,n,i;console.log("📋 Processing job:",t);const s={id:t.id,status:t.status||"pending",country:((a=t.parameters)==null?void 0:a.country)||t.country||"Unknown",platform:((o=t.parameters)==null?void 0:o.platform)||t.platform||"Unknown",genre:((n=t.parameters)==null?void 0:n.genre)||t.genre||"Unknown",contentType:((i=t.parameters)==null?void 0:i.contentType)||t.contentType||"Unknown",createdAt:t.createdAt||new Date().toISOString(),startedAt:t.startedAt,completedAt:t.completedAt,failedAt:t.failedAt,progress:t.progress||0,workerId:t.workerId,error:t.error};console.log("📋 Processed job data:",s),this.processData.set(t.id,s)}),console.log(`📋 Loaded ${e.jobs.length} jobs into process table`),this.updateProcessTable()):(console.log("📋 ProcessTable: No jobs received or API failed"),this.updateProcessTable())}catch(e){console.error("❌ Failed to load recent jobs:",e),r.addStatusMessage("error","❌","Failed to load job history"),this.updateProcessTable()}}updateProcessTable(){const e=document.getElementById("job-cards-container"),t=document.getElementById("empty-jobs-state"),s=document.getElementById("jobs-loading-state"),a=document.getElementById("job-count-badge");if(console.log("📊 JobDashboard: Updating dashboard..."),console.log("📊 JobDashboard: Cards container found:",!!e),console.log("📊 JobDashboard: Empty state found:",!!t),!e){console.warn("⚠️ Job cards container not found");return}if(s&&(s.style.display="none"),e.innerHTML="",console.log(`📊 JobDashboard: Processing ${this.processData.size} jobs`),a&&(a.textContent=this.processData.size),this.processData.size===0){t&&(t.style.display="block",console.log("📊 JobDashboard: Showing empty state"));return}t&&(t.style.display="none");const o=Array.from(this.processData.values()).sort((n,i)=>new Date(i.createdAt)-new Date(n.createdAt));o.forEach(n=>{const i=this.createJobCard(n);e.appendChild(i)}),console.log(`📊 JobDashboard: Updated with ${o.length} job cards`)}createJobCard(e){const t=document.createElement("div");t.className="col-lg-6 col-xl-4",this.getStatusClass(e.status);const s=this.getStatusIcon(e.status),a=e.id?e.id.slice(-8):"Unknown";let o="Not started",n="Not started",i="text-muted";if(e.startedAt){const S=new Date(e.startedAt);n=S.toLocaleString();const J=e.completedAt||e.failedAt||new Date,C=new Date(J)-S,$=Math.floor(C/6e4),D=Math.floor(C%6e4/1e3);$>0?o=`${$}m ${D}s`:o=`${D}s`,i="text-info"}else e.createdAt&&(n=`Created: ${new Date(e.createdAt).toLocaleString()}`,i="text-warning");const c=e.progress||0,p=c>=100?"bg-success":c>=50?"bg-info":"bg-warning";let f="border-secondary",b="bg-secondary";switch(e.status){case"pending":f="border-warning",b="bg-warning";break;case"active":case"processing":f="border-info",b="bg-info";break;case"completed":f="border-success",b="bg-success";break;case"failed":f="border-danger",b="bg-danger";break;case"cancelled":f="border-dark",b="bg-dark";break}return t.innerHTML=`
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="progress" style="height: 8px; background-color: #495057; border-radius: 10px;">
                            <div class="progress-bar ${p}" role="progressbar" 
        ...
        `,t}getStatusClass(e){return{pending:"bg-warning",active:"bg-info",completed:"bg-success",failed:"bg-danger",cancelled:"bg-secondary"}[e]||"bg-secondary"}getStatusIcon(e){return{pending:"⏳",active:"⚡",completed:"✅",failed:"❌",cancelled:"🚫"}[e]||"❓"}async viewJob(e){try{console.log(`👁️ Redirecting to job details page: ${e}`),window.location.href=`/job/${e}`}catch(t){console.error("❌ Failed to redirect to job details:",t),r.addStatusMessage("error","❌","Failed to open job details")}}async cancelJob(e){if(confirm(`Cancel job ${e.slice(-8)}?`))try{const t=await d.delete(`/api/queue/job/${e}`);if(t.success){r.addStatusMessage("success","✅","Job cancelled successfully");const s=this.processData.get(e);s&&(s.status="cancelled",this.updateProcessTable())}else throw new Error(t.error||"Failed to cancel job")}catch(t){console.error("❌ Failed to cancel job:",t),r.addStatusMessage("error","❌",`Failed to cancel job: ${t.message}`)}}async deleteJob(e){const t=this.processData.get(e),s=e.slice(-8),a=t?t.status:"Unknown";if(confirm(`⚠️ Permanently delete ${a} job ${s}?

This action cannot be undone!`))try{const o=await d.delete(`/api/queue/job/${e}/delete`);if(o.success)r.addStatusMessage("success","🗑️",`Job ${s} deleted successfully`),this.processData.delete(e),this.updateProcessTable();else throw new Error(o.error||"Failed to delete job")}catch(o){console.error("❌ Failed to delete job:",o),r.addStatusMessage("error","❌",`Failed to delete job: ${o.message}`)}}updateJobStatus(e){const t=this.processData.get(e.id);t&&(Object.assign(t,e),this.updateProcessTable())}addJob(e){this.processData.set(e.id,e),this.updateProcessTable()}startPeriodicUpdates(){this.stopPeriodicUpdates(),this.updateInterval=setInterval(()=>{console.log("📋 ProcessTable: Periodic backup refresh (anti-spam mode)"),this.loadRecentJobs()},6e5),console.log("📋 ProcessTable: Started 10-minute backup polling (anti-spam)")}stopPeriodicUpdates(){this.updateInterval&&(clearInterval(this.updateInterval),this.updateInterval=null)}getProcessData(){return this.processData}cleanup(){this.stopPeriodicUpdates(),this.processData.clear(),this.isInitialized=!1}}const E=new A;window.ProcessTable=E;class k{constructor(){this.currentRoute=null,this.navigationData={brand:{title:"StreamGank",subtitle:"AMBUSH THE BEST VOD TOGETHER",version:"BETA v1.3"},links:[{path:"/dashboard",label:"Dashboard",icon:"📊",description:"Video generation and queue management"},{path:"/jobs",label:"Jobs",icon:"📋",description:"View all jobs and their status"}]}}init(){this.setupRouterListener()}setupRouterListener(){h.addEventListener("routeChange",e=>{this.currentRoute=e.detail.path,this.updateActiveStates()})}render(e={}){const{showBrand:t=!0,showVersion:s=!0,showLogin:a=!0,fixed:o=!1,theme:n="default"}=e;return`
            <nav class="streamgank-navbar ${o?"fixed-top":""} ${n}">
...
Build: Production`)};const v=new k;class L{constructor(){this.isInitialized=!1}init(){this.isInitialized||(this.isInitialized=!0)}render(e){console.log("📊 Dashboard: Activating with existing HTML structure"),u.init(),r.init(),m.isInitialized||m.init(),console.log("📊 Dashboard functionality activated")}activate(){document.title="Dashboard - StreamGank Video Generator",m.refreshStatus(),console.log("📊 Dashboard activated")}deactivate(){console.log("📊 Dashboard deactivated")}getState(){const e=y&&typeof y.getFormData=="function",t=r&&typeof r.getState=="function";return{initialized:this.isInitialized,formData:e?y.getFormData():null,uiState:t?r.getState():null}}cleanup(){this.isInitialized=!1,console.log("📊 Dashboard Page cleaned up")}}const P=new L;class x{constructor(){this.currentJobId=null,this.jobData=null,this.refreshTimer=null,this.refreshInterval=5e3}async render(e,t={}){const{jobId:s}=t;if(!e){console.error("📄 JobDetail: No container provided");return}if(!s){console.error("📄 JobDetail: No job ID provided"),this.renderError(e,"No job ID specified");return}this.currentJobId=s,e.innerHTML=this.createLoadingTemplate();try{await this.loadJobData(s),e.innerHTML=this.createJobTemplate(),this.startAutoRefresh(),console.log(`📄 JobDetail rendered for job: ${s}`)}catch(a){console.error("📄 JobDetail render error:",a),this.renderError(e,a.message)}}async loadJobData(e){try{let t=g.getJob(e);if(!t){const s=await d.getJobStatus(e);s.success&&(t=s.job)}if(!t)throw new Error(`Job ${e} not found`);this.jobData=t,document.title=`Job ${e} - StreamGank`}catch(t){throw new Error(`Failed to load job data: ${t.message}`)}}createLoadingTemplate(){return`
        ...
        `}createJobTemplate(){var a,o,n,i,c,p;const e=this.jobData,t=this.getStatusClass(e.status),s=this.getStatusIcon(e.status);return`
            <div class="job-detail-page">
                <div class="container-fluid">
                            ...
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <dl class="row mb-0">
                                        ...
                                        <dd class="col-sm-8">${((n=e.params)==null?void 0:n.genre)||"N/A"}</dd>
        ...
        `}activate(e){console.log(`📄 JobDetail activated for job: ${e.jobId}`)}deactivate(){this.stopAutoRefresh(),console.log("📄 JobDetail deactivated")}getStatusClass(e){return{pending:"bg-warning",processing:"bg-info",completed:"bg-success",failed:"bg-danger",cancelled:"bg-secondary"}[e]||"bg-secondary"}getStatusIcon(e){return{created:"➕",started:"▶️",pending:"⏳",processing:"🔄",completed:"✅",failed:"❌",cancelled:"⏹️"}[e]||"📄"}getProgressClass(e){return e>=100?"bg-success":e>=75?"bg-info":e>=50?"bg-warning":"bg-primary"}formatDate(e){return e?new Date(e).toLocaleString():"N/A"}calculateDuration(e){if(!e.startedAt)return"Not started";const t=new Date(e.startedAt),a=(e.completedAt?new Date(e.completedAt):new Date)-t;return r.formatDuration(a)}cleanup(){this.stopAutoRefresh(),console.log("📄 JobDetail Page cleaned up")}}window.cancelJob=async l=>{if(confirm("Are you sure you want to cancel this job?"))try{await g.cancelJob(l)&&location.reload()}catch(e){alert(`Failed to cancel job: ${e.message}`)}};window.retryJob=l=>{alert("Retry functionality not yet implemented")};window.refreshJobData=l=>{location.reload()};window.checkCreatomateStatus=async l=>{try{const e=await d.getCreatomateStatus(l);e.success?alert(`Creatomate Status: ${e.status}
Progress: ${e.progress||"N/A"}%`):alert(`Error: ${e.message}`)}catch(e){alert(`Error checking status: ${e.message}`)}};window.copyToClipboard=async l=>{try{await navigator.clipboard.writeText(l),alert("URL copied to clipboard!")}catch{alert("Failed to copy URL")}};const R=new x;class F{constructor(){this.isInitialized=!1,this.currentPage=null,this.appContainer=null,this.modules={dom:u,router:h,api:d,realtime:m,jobs:g,ui:r,form:y,navigation:v},this.pages={dashboard:P,jobDetail:R}}async init(){if(this.isInitialized){console.warn("⚠️ App already initialized");return}try{await this.initializeCore(),this.setupAppContainer(),this.setupRouting(),await this.initializeServices(),await this.initializeComponents(),this.setupEventHandlers(),this.startServices(),this.isInitialized=!0}catch(e){console.error("❌ Failed to initialize app:",e),this.handleInitializationError(e)}}async initializeCore(){u.init()}setupAppContainer(){this.appContainer=document.body,this.mainContent=document.querySelector(".main-content")||document.querySelector(".container-fluid")||this.appContainer}setupRouting(){h.addRoute("/",()=>this.renderPage("dashboard"),{title:"Dashboard - StreamGank Video Generator"}),h.addRoute("/dashboard",()=>this.renderPage("dashboard"),{title:"Dashboard - StreamGank Video Generator"}),h.addRoute("/job/:jobId",e=>this.renderPage("jobDetail",e),{title:"Job Details - StreamGank Video Generator"}),h.addRoute("/jobs",()=>this.renderJobsPage(),{title:"All Jobs - StreamGank Video Generator"}),h.addEventListener("routeChange",e=>{}),h.addEventListener("notFound",e=>{console.warn(`🛤️ Route not found: ${e.detail.path}`),h.navigate("/dashboard",{replace:!0})})}async initializeServices(){g.init()}async initializeComponents(){v.init(),await E.init(),P.init()}setupEventHandlers(){document.addEventListener("formSubmit",e=>{this.handleFormSubmission(e.detail)}),g.addEventListener("jobStarted",e=>{console.log("💼 Job started:",e.detail.job.id)}),g.addEventListener("jobCompleted",e=>{console.log("✅ Job completed:",e.detail.job.id)}),g.addEventListener("jobFailed",e=>{console.error("❌ Job failed:",e.detail.job.id,e.detail.error)}),m.addEventListener("connected",e=>{console.log("📡 Real-time connection established:",e.detail.type),v.updateStatus({connected:!0,connectionType:e.detail.type})}),m.addEventListener("disconnected",e=>{console.warn("📡 Real-time connection lost:",e.detail.type),v.updateStatus({connected:!1,connectionType:e.detail.type})}),m.addEventListener("queueUpdate",e=>{v.updateStatus({connected:!0,queue:e.detail.stats})}),window.addEventListener("nav-refresh-requested",()=>{m.refreshStatus()}),v.setupEventHandlers()}setupButtonHandlers(){const e=u.get("refreshQueueBtn");e&&e.addEventListener("click",()=>{m.refreshStatus()});const t=u.get("clearQueueBtn");t&&t.addEventListener("click",async()=>{if(confirm("Are you sure you want to clear the entire queue? This will cancel all pending jobs."))try{(await d.clearQueue()).success?r.addStatusMessage("success","✅","Queue cleared successfully"):r.addStatusMessage("error","❌","Failed to clear queue")}catch(o){r.addStatusMessage("error","❌",`Error clearing queue: ${o.message}`)}});const s=u.get("checkStatusBtn");s&&s.addEventListener("click",()=>{this.handleManualStatusCheck()});const a=u.get("loadVideoBtn");a&&a.addEventListener("click",()=>{this.handleManualVideoLoad()})}startServices(){h.init(),m.init()}async renderPage(e,t={}){try{if(this.currentPage&&this.pages[this.currentPage]&&typeof this.pages[this.currentPage].deactivate=="function"&&this.pages[this.currentPage].deactivate(),e==="dashboard")r.init(),await y.init(),window.FormManager=y,this.pages[e]&&typeof this.pages[e].activate=="function"&&this.pages[e].activate(t),this.currentPage=e;else if(e==="jobDetail")await this.renderJobDetailPage(t),this.currentPage=e;else throw new Error(`Page '${e}' not found`)}catch(s){console.error(`🎨 Failed to render page '${e}':`,s),alert(`Error: ${s.message}`)}}async renderJobDetailPage(e){const{jobId:t}=e,s=`
        ...
        `;let a=document.getElementById("jobDetailModal");a||(document.body.insertAdjacentHTML("beforeend",s),a=document.getElementById("jobDetailModal")),new bootstrap.Modal(a).show();try{const n=document.getElementById("jobDetailContent");this.pages.jobDetail&&typeof this.pages.jobDetail.render=="function"&&await this.pages.jobDetail.render(n,e)}catch(n){console.error("Failed to load job details:",n),document.getElementById("jobDetailContent").innerHTML=`
                <div class="alert alert-danger">
                    <h6>Error</h6>
                    <p>Failed to load job details: ${n.message}</p>
                </div>
            `}a.addEventListener("hidden.bs.modal",()=>{h.navigate("/dashboard")})}renderJobsPage(){const e=document.getElementById("app-content");e&&(e.innerHTML=`
            <div class="jobs-page">
                <div class="container-fluid">
                    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        `}async handleFormSubmission(e){const{formData:t,previewUrl:s,validation:a}=e;console.log("📋 Form submitted:",t);try{const o={country:t.country,platform:t.platform,genre:t.genre,template:t.template,contentType:t.contentType,url:s};await g.startVideoGeneration(o),console.log("🔄 Refreshing dashboard to show new job..."),window.ProcessTable&&typeof window.ProcessTable.loadRecentJobs=="function"&&setTimeout(()=>{window.ProcessTable.loadRecentJobs()},1e3)}catch(o){console.error("❌ Form submission failed:",o),r.addStatusMessage("error","❌",`Generation failed: ${o.message}`)}}async handleManualStatusCheck(){const e=u.get("creatomateIdDisplay");if(!e||!e.textContent){r.addStatusMessage("warning","⚠️","No Creatomate ID available for status check");return}const t=e.textContent.trim();try{r.addStatusMessage("info","🔍","Checking render status...");const s=await d.getCreatomateStatus(t);if(s.success&&s.videoUrl)r.addStatusMessage("success","🎬","Video is ready!"),r.displayVideo({jobId:`manual-${Date.now()}`,videoUrl:s.videoUrl,creatomateId:t});else if(s.success&&s.status){const a=s.status.charAt(0).toUpperCase()+s.status.slice(1);r.addStatusMessage("info","⏳",`Render status: ${a}`)}else r.addStatusMessage("error","❌",`Status check failed: ${s.message||"Unknown error"}`)}catch(s){console.error("❌ Manual status check failed:",s),r.addStatusMessage("error","❌",`Status check error: ${s.message}`)}}handleManualVideoLoad(){const e=prompt("Enter Creatomate render ID:");if(!e||!e.trim()){r.addStatusMessage("warning","⚠️","No render ID provided");return}const t=e.trim(),s=u.get("creatomateIdDisplay");s&&(s.textContent=t),r.addStatusMessage("info","📥",`Loading video for render ID: ${t}`),this.handleManualStatusCheck()}handleInitializationError(e){const t=`Failed to initialize application: ${e.message}`;try{r.addStatusMessage("error","❌",t,!1)}catch{console.error("❌",t),alert(t)}}getStatus(){return{initialized:this.isInitialized,modules:Object.fromEntries(Object.entries(this.modules).map(([e,t])=>[e,typeof t.getStatus=="function"?t.getStatus():"ready"])),realtime:m.getConnectionStatus(),jobs:g.getJobStats(),api:d.getCacheStats()}}async restart(){console.log("🔄 Restarting application..."),this.cleanup(),this.isInitialized=!1,setTimeout(async()=>{await this.init()},1e3)}stopAllPolling(){console.log("🛑 ANTI-SPAM: Stopping all polling timers..."),window.ProcessTable&&typeof window.ProcessTable.stopPeriodicUpdates=="function"&&window.ProcessTable.stopPeriodicUpdates(),console.log("✅ All polling stopped (anti-spam mode)")}cleanup(){console.log("🧹 Cleaning up application resources..."),this.stopAllPolling(),Object.values(this.modules).forEach(e=>{typeof e.cleanup=="function"&&e.cleanup()}),console.log("✅ Application cleaned up")}}const w=new F;document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{w.init()}):w.init();window.StreamGankApp=w;
//# sourceMappingURL=main.CN0nO4Zh.js.map
