...
app.get('/api/job/:jobId/logs', async (req, res) => {
    try {
        const { jobId } = req.params;
        // Reduced logging to avoid cluttering Docker logs
        const logs = queueManager.getJobLogs(jobId);

        // Only log when there are new logs to avoid spam
        if (logs.length > 0) {
            // Only log once every 10 requests or when log count changes significantly
            const logCountKey = `lastLogCount_${jobId}`;
            const lastCount = global[logCountKey] || 0;

            if (logs.length !== lastCount && logs.length - lastCount >= 2) {
                console.log(`📝 Job ${jobId.slice(-8)}: ${logs.length} total logs (+${logs.length - lastCount} new)`);
                global[logCountKey] = logs.length;
            }
        }
